...
\hypersetup{
  ...
  citecolor=OceanBlue,
  urlcolor=OceanBlue,
  pdftitle={亚非拉国别风险与多币种储备智能决策平台总计划书},
  pdfsubject={数据采集、处理、验收与Smartbi落地},
  pdfauthor={数据创新平台项目组}
}
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small 亚非拉国别风险与多币种储备智能决策平台总计划书}
...
\begin{titlepage}
  ...
  {\Huge\bfseries\color{DeepNavy}亚非拉国别风险与多币种储备\\[0.35cm]智能决策平台总计划书\par}
  \vspace{0.7cm}
  {\LARGE\color{OceanBlue}数据采集、处理、验收与 Smartbi 落地\par}
  \vspace{1.0cm}
  \begin{minipage}{0.86\textwidth}
    \color{GoldLine}\rule{\textwidth}{0.9pt}\par\vspace{0.45cm}
    \color{black}\centering
    130 国覆盖层｜40 国月度量化层｜20 家中国企业层\\[0.25cm]
    2010 年 1 月—2025 年 12 月｜192 个月\\[0.25cm]
    Python 可复现处理｜标准 XLSX｜Smartbi 五页看板｜AIChat 验收
...
本册是一份可执行的研究与实施总计划。它不把尚未采集的数据、尚未运行的模型或尚未建设的 Smartbi 看板写成既成成果，而是把每一步需要的输入、操作、输出文件、数据主键、质量阈值、失败处理和交接对象固化为团队合同。3—4 名成员应当能够仅凭本册完成任务拆分，并判断每一阶段是否具备进入下一阶段的资格。
\end{decisionbox}

项目研究对象是中国出海企业在亚洲、非洲和拉丁美洲经营时面临的汇率贬值、通胀、外汇储备下降、资本与利润汇出限制、支付渠道中断、制裁与托管约束。产品不预设美元或黄金为单一胜者，而是把经营现金、流动性储备和战略储备分层，在人民币、经营国本币与合同货币三种计价口径下，对人民币短期资产、美元现金、美元短债、黄金、套保、自然对冲和授信进行周期协同实验。

数据结构由四个层次组成：项目定义的亚非拉 130 国用于展示覆盖广度和投资版图；按明确规则筛选的 40 国承担 192 个月的月度量化分析；20 家中国企业承担公开披露画像；不少于 20 个独立危机事件由 40 国面板与权威事件证据共同识别。历史 45 份案例全部转入证据矩阵，但重复事件通过 \Field{event_cluster_id} 合并，不把文档数量当作独立事件数量。

端到端流程固定为：D0 研究口径冻结，D1 建立 130 国主数据，D2 登记来源并下载原始文件，D3 构建投资覆盖表，D4 采集候选宏观数据，D5 筛选并冻结 40 国，D6 采集政策、事件、企业和资产数据，D7 标准化、清洗与质量检查，D8 识别危机事件与周期状态，D9 构建资产收益和组合情景，D10 导出 Smartbi 数据包，D11 建模、看板与 AIChat，D12 完成 XML 恢复、报告、视频和提交。任何门槛失败都不得通过填零、降低阈值、拆分事件或伪造数据完整性来绕过。

2026 年 8 月 16 日对主办方账号完成了不写入资源的认证与能力盘点：实例为 Smartbi Insight Edition V11，已开放增强数据集、导入表/Excel、自助 ETL、指标模型、可视化看板、AIChat、项目、知识库、智能体及资源导出/迁移相关能力。结合官方 V11、数据准备与 AIChat 说明\cite{smartbiversion,smartbiprepare,smartbiaichat}，本项目的主路径据此冻结为“Python 权威 XLSX 包—导入表/Excel—星座型增强数据模型—统一指标模型—五页看板—AI 项目/知识库/智能体—XML 导出与隔离恢复”。上传、建模、看板、AIChat 数值测试和 XML 恢复尚未执行，必须先通过平台小样试跑，不能用本次只读盘点替代产品验收。
...
本册独立维护参考文献，不调用也不修改历史 \File{references.bib}。历史 V1.1、V2.0 与旧 Mermaid 保持原状，仅作为项目演进记录。2025 年数据若在执行时尚无正式值，则保持为空并报告最新完整期，不得用预测值冒充实际值。
...
20 个问题必须全部执行并留存录屏。数值题正确率不低于 90\%，关键数值误差不超过 1\%；集合题按精确匹配验收；任何企业国家粒度越级推断、来源缺失、模拟冒充真实或禁止路径输出，均属于硬失败而非普通文字瑕疵。
\end{gatebox}
...
\caption{四层研究对象及冻结条件}\label{tab:sample-contract}\\
\toprule
...
\begin{enumerate}
  ...
  \item 九张权威逻辑表、六张维度表、一张企业—地区桥接表、数据字典、导入清单和模型映射；
  \item 40 国风险面板、危机事件、周期状态、资产收益与组合情景结果；
  \item Smartbi 星座型增强数据模型 \Field{MDL_XH202612_COUNTRY_RESERVE_V31}、指标模型 \Field{IM_XH202612_COUNTRY_RESERVE_V31}、五页看板、AI 项目/知识库/智能体与 20 个 AIChat 测试卡；
...
G3 数据门 & D7 后 & 九表主键、频率、单位、来源、完整率达到阈值 & 问题行隔离到 QA，不进入正式模型；补采后重跑。\\
G4 模型门 & D9 后 & 事件、收益分解、无未来信息、成本和对称实验可复算 & 删除超出证据的普遍性结论，不降低事件阈值。\\
P0 平台门 & D10 前 & 三表小样、类型、连接、KPI、折线、筛选、三问和 XML 导出完成 & 不批量导入；先修复平台路径或记录许可/恢复阻塞。\\
G5 产品门 & D11 后 & 五页联动、20 问、性能与 Python 对账通过 & 修正模型/指标后重新导入，禁止在看板手工改数。\\
...
共享账号内任何通用名称都可能与他人资源冲突。所有项目对象统一前缀 \Field{XH202612_V31_}，显示名可以使用中文，但资源内部名和导入文件名必须保持唯一。A 在创建前登记“对象名—创建人—修改人—依赖—导出批次”，同一对象同一时段只允许一名写入者。

\begin{longtable}{p{3.4cm}p{5.4cm}p{5.2cm}}
\caption{Smartbi V3.1 权威资源命名}\label{tab:smartbi-resource-names}\\
\toprule
\textbf{对象} & \textbf{固定名称} & \textbf{作用与依赖}\\
\midrule
隔离项目/目录 & \Field{XH202612_V31} & 只承载本项目新资源；不接管账号中任何既存对象。\\
增强数据模型 & \Field{MDL_XH202612_COUNTRY_RESERVE_V31} & 星座模型；共享国家、月、年、企业、资产、事件六类维度。\\
指标模型 & \Field{IM_XH202612_COUNTRY_RESERVE_V31} & 固化指标代码、中文名、公式、默认聚合、计价口径、时间粒度和来源。\\
五页看板 & \Field{DB01_XH202612_RISK_OVERVIEW_V31} 至 \Field{DB05_XH202612_DECISION_EVIDENCE_V31} & 依次对应风险全景、国别事件、企业暴露、周期实验室、决策证据。\\
AI 项目 & \Field{PRJ_XH202612_COUNTRY_RESERVE_V31} & 绑定指标模型、知识库和测试问题，不直接面向未治理的原始事实表。\\
知识库 & \Field{KB_XH202612_COUNTRY_RESERVE_V31} & 收纳术语、数据字典、危机规则、来源等级、约束路由和 45 案例摘要。\\
智能体 & \Field{AGT_XH202612_COUNTRY_RESERVE_V31} & 只回答经授权粒度的问题；输出指标口径、截止期、来源和限制。\\
恢复包 & \Field{PKG_XH202612_COUNTRY_RESERVE_V31} & XML、依赖清单、数据包、映射表、版本说明、哈希和恢复记录的统一批次。\\
\bottomrule
\end{longtable}

\section{输出格式、模型关系与调用合同}

Python 输出分为 16 个可导入数据工作簿和 3 个控制工作簿。16 个数据工作簿由九张权威逻辑表、六张维度表和一张企业—地区桥表构成；新增 \Field{dim_year.xlsx} 专门连接年度投资、政策和企业披露，禁止把年度值复制成 12 个月。三个控制工作簿是 \textsf{数据字典}\File{_V3.1.xlsx}、\File{Smartbi}\textsf{导入行数与哈希清单}\File{.xlsx} 和 \File{Smartbi}\textsf{模型映射}\File{_V3.1.xlsx}。

\File{Smartbi}\textsf{模型映射}\File{_V3.1.xlsx} 至少包含 \Field{import_order}、\Field{file_name}、\Field{sheet_name}、\Field{smartbi_resource_name}、\Field{grain}、\Field{primary_key}、\Field{field_name}、\Field{dimension_measure_role}、\Field{data_type}、\Field{relationship}、\Field{dashboard_page}、\Field{ai_metric_code}、\Field{refresh_mode}、\Field{source_id} 和 \Field{run_id}。团队不得只凭口头约定在平台中猜字段角色。

星座模型的连接纪律如下：\Field{dim_country} 连接国家事实；\Field{dim_date} 只连接真实月度事实；\Field{dim_year} 连接年度投资、政策和企业披露；\Field{dim_company}、\Field{dim_asset}、\Field{dim_event} 分别连接对应事实；\Field{bridge_company_geography} 只表达企业公开披露到国家或地区的真实粒度。九张事实/逻辑表之间不直接连接，区域披露不得展开为具体国家。\Field{source_registry} 作为追溯参考对象，由 \Field{source_id} 从指标、图表和 AI 答案一跳访问。
...
在完整 D10 导入前，A 必须取得项目组对写入范围的确认，然后只在 \Field{XH202612_V31} 隔离目录创建一个可删除的小样批次。小样包含 \Field{dim_country}、\Field{dim_date} 和 \Field{country_monthly_risk} 三张脱敏示例表，覆盖至少 3 国、24 个月、汇率/CPI/来源/质量字段。依次验证 Excel 类型识别、三表连接、一个 KPI、一个折线图、一个国家筛选器、三条 AI 问题、XML 导出和可用目标空间中的恢复。
...
\CardRow{输入}{赛题原文、现有 45 份案例、历史计划书、商务部公报结构、候选指标清单、Smartbi 试用环境约束。}
\CardRow{操作}{建立 \File{config/project_scope_v31.yaml}、\File{config/indicator_catalog_v31.xlsx}、\File{config/company_master_v31.xlsx} 和 \File{config/source_priority_v31.xlsx}。逐项冻结研究对象、日期边界、币种方向、原始频率、主键、事件阈值、三层储备、三种计价、禁止使用未来信息规则和 QA 阈值。所有变更通过变更单记录提出人、理由、影响表、批准人和生效运行号。}
\CardRow{输出路径}{\File{config/} 下四份配置；\File{data/qa/D0_scope_signoff_20260815.xlsx}；范围冻结纪要。}
\CardRow{验收}{130、40、20、192 个月等数字在全部配置中一致；汇率方向明确为本币/美元；年度指标不得进入月度主键；每个字段有数据类型、频率、单位和责任人；四名角色或实际三名成员完成签字。}
...
\CardRow{输入}{\File{config/source_priority_v31.xlsx}、各机构数据页面/API、许可证或使用说明、截至日期 2025-12-31。}
\CardRow{操作}{先分配 \Field{source_id}，后下载。每个来源记录发布机构、数据集、URL、许可、适用范围、频率、覆盖期、发布日期、抓取时间、最早可用于回测日期和替代源。原始文件保存到 \File{data/raw/<source_id>/<fetch_date>/}，计算 SHA-256；API 响应同时保留请求参数、状态码、分页信息和原始 JSON。网页证据保存 PDF 或 HTML 时记录标题和访问日期。}
\CardRow{输出路径}{\File{data/curated/source_registry.xlsx} 初版；\File{data/raw/} 原始文件；\File{data/qa/download_log_<run_id>.xlsx}。}
\CardRow{验收}{每个原始文件有 source ID、抓取时间、文件大小和 SHA-256；许可状态不是“未知”；API 页数与响应行数一致；禁止覆盖相同路径；所有失败请求有错误类型与重试次数。}
\CardRow{失败处理}{HTTP 限流使用指数退避并保留日志；访问受限时不绕过限制，转登记替代公开源；需人工接受条款的来源由 A 决定是否纳入，未获授权不得进入正式成果。}
...
\CardRow{输出路径}{\File{config/selected_40_countries_v31.csv}、\File{data/qa/country_selection_scorecard.xlsx}、\File{data/qa/excluded_country_reasons.xlsx}。}
...
\CardRow{输出路径}{\File{templates/policy_event_manual_entry_v31.xlsx}、\File{data/staging/country_policy_coded.parquet}、\File{data/curated/country_policy_year.xlsx}。}
...
\CardRow{输出路径}{\File{templates/company_disclosure_v31.xlsx}、\File{data/staging/company_disclosure_checked.parquet}、\File{data/curated/company_overseas_exposure.xlsx}。}
...
\CardRow{输入}{40 国清单、美元金价、上海金、美元兑人民币、3 个月美债代理、人民币短期资产、可信平行汇率、交易和托管成本、45 份案例。}
\CardRow{操作}{资产序列由 B 维护；事件和案例证据由 D 维护。对每种资产记录市场、币种、单位、交易日历、月度聚合、总回报/价格回报、税费是否计入。平行汇率只用于可核验国家并与官方汇率分列。45 份案例填入统一模板，标记支持、反驳、限定或冲突，重复观察通过事件簇归并。}
\CardRow{输出路径}{\File{templates/case_evidence_v31.xlsx}、\File{templates/event_evidence_v31.xlsx}、\File{data/staging/asset_prices_standardized.parquet}、\File{data/curated/case_evidence.xlsx}。}
\CardRow{验收}{核心美元金价、美元短债和美元兑人民币月度序列完整率不低于 95\%；45 份案例全部入表；每个案例有 source ID 和角色；同一事件簇不重复计数；成本来源、适用资产、计价单位和有效期明确。}
...
\StepTitle{D7}{标准化与九表质量门}{负责人：B；独立验证：C；人工字段复核：D}
\CardRow{输入}{D3—D6 全部 staging 数据、国家别名、币制改制、单位转换、日期和来源配置。}
\CardRow{操作}{运行 \File{scripts/07_standardize_validate.py}。映射 ISO3；汇率统一本币/美元；日期统一月末；区分空、N/A、字符串零和真实零；年度与月度分表；极端值只标记不删除；币制改制使用转换表；企业和政策人工字段验证页码与复核人；所有正式数值检查 source ID。}
\CardRow{输出路径}{\File{data/curated/} 九张权威表的待建模版；\File{data/qa/D7_quality_summary.xlsx}、\File{data/qa/D7_rejected_rows.xlsx}、\File{data/qa/D7_frequency_audit.xlsx}。}
\CardRow{验收}{九表主键重复率 0、主键空值率 0；正式数值 source ID 覆盖率 100\%；40 国汇率和 CPI 各满足 154/192；频率审计无年度伪月度；货币改制机械跳点不被当危机；代理、模拟、插值和真实记录可分别筛选。}
...
\CardRow{操作}{运行 \File{scripts/09_detect_crisis_events.py}。任一规则触发即生成候选：12 个月本币贬值不低于 20\%；CPI 同比不低于 15\%；官方与可信平行汇率差不低于 20\%；资本管制明显收紧；资金冻结、汇出限制或银行系统中断。同国同类触发连续出现或间隔不超过 2 个月时合并；连续 3 个月退出所有数值阈值或正式限制解除后结束。建立前 12 月、事件期和后 24 月窗口。}
...
\newpage
\section{D8B 周期状态识别}
\StepTitle{D8B}{周期标签与挑战模型}{负责人：C；解释复核：A}
\CardRow{输入}{实际利率、美元趋势、全球与当地通胀、汇率压力、资本限制、金融渠道状态和事件标记。}
\CardRow{操作}{先用预注册规则划分正常、美元偏强/实际利率上行、通胀与实际利率下行、本币压力、资本与支付受限、复合危机等状态；再用聚类或隐马尔可夫模型作为挑战。规则模型使用决策时可知的滚动指标，挑战模型不得利用完整样本的未来中心值为历史月份贴标签。}
\CardRow{输出路径}{\File{data/staging/cycle_state_rule.parquet}；\par\File{data/staging/cycle_state_challenge.parquet}；\par\File{data/qa/cycle_disagreement.xlsx}。}
\CardRow{验收}{每个状态有明确阈值和经济含义；规则与挑战差异可追溯；标签发布日期不晚于组合决策日；不存在“看完收益再改状态”的后验调参。}
\CardRow{失败处理}{挑战模型不稳定时仅保留规则主模型；状态样本过少时合并为上位类别并记录理由，不删除亏损窗口。}
\CardRow{下一步接收人}{D9 将周期状态作为情景维度，不直接当作未来可准确预测的交易信号。}
...
\CardRow{输入}{企业画像、国家与事件、周期状态、资产收益、成本、三层储备、托管与约束状态。}
\CardRow{操作}{运行 \File{scripts/10_run_portfolio_scenarios.py}。对 90 天、1 年、5 年和 10 年期限构造本币、人民币短期资产、美元、黄金、套保组合。战略层测试 0\%、2.5\%、5\%、7.5\%、10\%、15\%、20\% 黄金网格；2.5\% 仅在期限、托管、权属和路径资格通过时作为条件底仓。每个完整组合运行“去黄金”和“去美元资产”对称实验。}
...
\CardRow{验收}{权重和为 1，误差不超过 \(10^{-8}\)；成本非负；无未来信息；90 天覆盖率、人民币购买力、最大回撤、恢复时间、95\% CVaR、合法可动用比例和综合成本均可重算；至少保留 3 个黄金不占优和 3 个美元短债不占优窗口。}
\CardRow{失败处理}{找不到足够反例时不人为挑选日期，改为报告“在本样本与成本设定下未满足反例覆盖门槛”；约束禁止的资产权重强制为 0，并记录约束原因，但继续输出风险诊断与可行替代类别。}
\CardRow{下一步接收人}{D10 导出 Smartbi 包；D11 第四、五页消费组合结果。}
...
\CardRow{输入}{G4 通过的九张权威表、国家/月/年/企业/资产/事件维度规则、企业—地区关系，以及表\ref{tab:smartbi-resource-names}的资源命名合同。}
\CardRow{操作}{运行 \File{scripts/11_export_smartbi_workbooks.py}。九张权威表各导出一个 XLSX；另导出 \Field{dim_country}、\Field{dim_date}、\Field{dim_year}、\Field{dim_company}、\Field{dim_asset}、\Field{dim_event} 和 \Field{bridge_company_geography}。\Field{dim_date} 只管理真实月度日期，\Field{dim_year} 管理年度投资、政策与披露。每个数据文件只含名为 \Field{data} 的工作表；第一行使用英文蛇形字段名；日期、整数、小数和布尔值保存为真实 Excel 类型；中文释义只放在数据字典。}
\CardRow{输出路径}{16 个 \File{data/smartbi/*.xlsx} 数据文件，以及 \File{data/smartbi/}\textsf{数据字典}\File{_V3.1.xlsx}、\File{data/smartbi/Smartbi}\textsf{导入行数与哈希清单}\File{.xlsx}、\File{data/smartbi/Smartbi}\textsf{模型映射}\File{_V3.1.xlsx}。}
\CardRow{验收}{16 个数据文件和 3 个控制文件齐全；数据工作表名均为 data；字段顺序与数据字典、字段角色与模型映射一致；导出前后行数一致；文件 SHA-256、行列数、主键重复数、空主键数写入清单。}
...
\Field{company_overseas_exposure.xlsx} & 企业—财年—披露地区 & \Field{company_id+fiscal_year+geography_id} & 11\\
\Field{bridge_company_geography.xlsx} & 企业披露—地理对象 & \Field{bridge_id} & 12\\
\Field{asset_monthly_return.xlsx} & 国家—月份—资产—口径 & \Field{iso3+month_end+asset_id+currency_basis} & 13\\
\Field{portfolio_scenario.xlsx} & 情景—策略—运行版 & \Field{scenario_id+strategy_id+run_id} & 14\\
\Field{case_evidence.xlsx} & 案例证据 & \Field{case_id} & 15\\
\Field{source_registry.xlsx} & 来源 & \Field{source_id} & 16\\
\bottomrule
\end{longtable}

\newpage
\section{D11 建模、五页看板和 AIChat}
\StepTitle{D11}{Smartbi 业务模型与交互产品}{负责人：A；数据对账：B；问题验收：D}
\CardRow{输入}{D10 的 16 个数据工作簿、3 个控制工作簿、指标公式、五页线框、20 个 AI 问题测试卡，以及 P0 小样通过证据。}
\CardRow{操作}{在 \Field{XH202612_V31} 隔离目录中，按映射表通过“导入表/Excel”建立数据对象；规则式导入不可用时才回退到自助 ETL 的 Excel/导入文件节点，业务主题只作为兼容回退。建立星座型增强数据模型 \Field{MDL_XH202612_COUNTRY_RESERVE_V31}，由六张维度共享连接九张事实/逻辑表；事实表之间禁止直接多对多。随后建立 \Field{IM_XH202612_COUNTRY_RESERVE_V31}，固化指标代码、中文别名、默认聚合、计价口径和时间粒度；再创建五页看板、AI 项目、知识库和智能体。}
\CardRow{输出路径}{表\ref{tab:smartbi-resource-names}所列平台资源；\File{data/qa/smartbi_import_reconciliation.xlsx}；\File{data/qa/smartbi_resource_lock.xlsx}；五页看板；AIChat 测试录屏与结果表。}
\CardRow{验收}{导入前后逐表行数一致；主外键覆盖率达表级阈值且无意外膨胀；五页筛选、联动、下钻可用；典型操作不超过 10 秒；Python 与 Smartbi 关键指标误差不超过 1\%；20 问数值题正确率不低于 90\%、关键误差不超过 1\%。}
...
日期维度 & 月度风险、资产、事件窗口、情景 & \Field{month_end}、\Field{date_key} & 月度事实覆盖率 100\%。\\
...
事件维度 & 事件窗口、情景、案例关联 & event\_id & 正式事件覆盖率 100\%。\\
...
\CardRow{输入}{G5 通过的 Smartbi 模型和看板、AIChat 录屏、权威数据包、QA 报告、来源与合规说明。}
\CardRow{操作}{导出项目目录、星座型增强数据模型、指标模型、五页看板、AI 项目、知识库、智能体及其数据对象的 XML/迁移资源与依赖清单；在未加载本项目资源的隔离目标恢复，记录环境版本、导入步骤、耗时、警告和截图；执行 10 个关键看板路径与 20 个 AI 问题的恢复后抽验。完成分析报告、三分钟以内视频、操作指南、示例数据、字段字典、来源说明、合规声明和最终哈希清单。运行 \File{scripts/12_generate_qa_report.py} 汇总全链路验收。}
\CardRow{输出路径}{\File{submission/Smartbi_XML/}、\File{submission/report/}、\File{submission/video/}、\File{submission/data/}、\File{submission/docs/}、\File{submission/final_manifest.xlsx}。}
\CardRow{验收}{XML 干净恢复成功；五页和关键指标可用；视频不超过 3 分钟；报告中的关键数字全部可追溯；提交包无临时锁文件和绝对本机路径；最终清单记录每个文件大小与 SHA-256。}
...
\File{07_standardize_validate.py} & ISO3、日期、频率、单位、缺失、来源与主键验证 & 九表候选、拒绝行、质量报告；\Field{G3_FAIL}。\\
...
\File{11_export_smartbi_workbooks.py} & 导出 16 个 data 工作表 XLSX 与三份控制工作簿 & Smartbi 包、行数/哈希清单、数据字典和模型映射；\Field{EXPORT_SCHEMA_FAIL}。\\
...
\begin{verbatim}
python scripts/run_pipeline.py --as-of 2025-12-31 --run-id 20260831_v31
\end{verbatim}
...
正式提交采用单一冻结运行号。冻结并不意味着人工复制最终 Excel，而是锁定配置、来源快照和脚本版本后从原始层重跑。任何成员应能在相同依赖下重建九张权威表和 Smartbi 导入包；若外部 API 后续修订，原始快照保证历史运行仍可复现，更新数据则使用新运行号。

\part{九张权威表与完整字段字典}

\chapter{字段字典使用规则}

后续九章给出权威表的完整最低字段合同。实际执行可以增加调试或来源原字段，但不得删除主键、追溯、质量和运行字段。所有结果表必须保留 \Field{source_id}、\Field{vintage_date}、\Field{is_proxy}、\Field{is_simulated}、\Field{is_imputed}、\Field{quality_flag} 与 \Field{run_id} 中适用的字段。本币、美元和人民币口径通过 \Field{currency_basis} 或独立数值字段表达，不覆盖原始数值。
...
\Field{cycle_state} & 枚举 & 是 & -- & 决策时可知的规则状态。\\
...
\begin{gatebox}[表级验收]
主键 \Field{scenario_id+strategy_id+run_id} 重复率 0；每个策略权重和误差不超过 \(10^{-8}\)；成本均非负；\Field{lookahead_flag=1} 的正式行数为 0；禁止状态下受限资产权重为 0；完整、去黄金和去美元组合成组出现并可复算。
\end{gatebox}
...
\begin{gatebox}[表级验收]
所有进入看板的正式数值都能连接 source ID；原始文件路径、大小和 SHA-256 齐全；许可未知数为 0；回测时使用的观测版本不早于其 \Field{earliest_backtest_date}；代理和模拟来源可单独筛选。
\end{gatebox}

\part{Smartbi 五页产品、指标与 AIChat}
...
DQ13 & 九表主键 & group by 主键 having count>1 & 0 & duplicate report & B/C\\
DQ14 & 九表空主键 & 主键任一列为空 & 0 & key-null report & B/C\\
...
\begin{enumerate}
  \item 先查重并创建隔离目录 \Field{XH202612_V31}，登记资源锁；不得复用或修改登录前已有对象；
  \item 在 P0 只导入三张小样表，取得表\ref{tab:smartbi-instance-audit}未验证项的实测证据；P0 不通过不得批量导入；
  \item 主路径使用增强数据集中的“导入表/Excel”，按表\ref{tab:import-package}先导入六张维度，再导入九张权威表和桥表；
  \item 若“导入表/Excel”不满足类型或更新要求，回退到自助 ETL 的 Excel/导入文件节点；业务主题只作为版本兼容回退，不作为主模型；
  \item 每个对象采用表\ref{tab:smartbi-resource-names}和 \File{Smartbi}\textsf{模型映射}\File{_V3.1.xlsx}的唯一名称、角色、连接与刷新方式；
...
P07 & AIChat 执行包含来源追溯的数值问答 & 记录，不与看板阈值混用 & 首 token/总耗时与准确率。\\
\bottomrule
\end{longtable}

性能不达标时按顺序处理：删除不必要高基数字段的默认展示；建立按月/国/企业的预聚合；检查多对多和无条件笛卡尔连接；减少一次页面返回的明细行；必要时对事实按年份或运行号分区。实测服务资源快照只能帮助确定测试规模，不能证明性能达标；P01—P06 均需三次冷/热缓存测试并记录 P50/P95。不得以删除质量字段、来源追溯或反例数据换取性能。

\chapter{XML 干净恢复测试}

\section{干净环境定义}

干净环境是未加载 \Field{XH202612_V31} 项目数据、模型、指标、看板、知识库、智能体和 AIChat 配置的新租户或经主办方确认的隔离目标。可以保留产品本身和必要账号，但不得预先存在同名前缀对象。同一非空工作区内覆盖旧对象不算干净恢复。恢复人最好不是原搭建人，以验证操作指南是否足够；若主办方不给第二目标空间，必须把恢复标为未验收并说明外部依赖。

\section{恢复步骤}

\begin{enumerate}
  \item 记录源环境和目标环境的 Smartbi 版本、补丁、许可模块和时间；
  \item 导出项目、增强数据模型、指标模型、五页看板、AI 项目、知识库、智能体及数据对象的 XML/迁移资源、依赖清单和连接说明，计算 SHA-256；
  \item 在目标环境导入 XML，记录所有警告和对象映射；
  \item 按操作指南连接或导入 V3.1 的 16 个数据工作簿与 3 个控制工作簿，不使用源环境本机绝对路径；
  \item 验证六张维度、九张权威表和桥表的连接，特别检查月度与年度事实未混频；
  \item 打开五页并执行 P01—P06；
  \item 执行 AI-01、AI-07、AI-11、AI-14、AI-17、AI-18、AI-19 的关键抽验；
  \item 比较源环境和恢复环境的 20 个关键 KPI；
...
\endhead
8/15 & 冻结 V3.0 数据、目录和角色基线 & 核对 130 国和指标源 & 固定事件/组合公式 & 固定企业与案例模板\\
8/16 & 完成 Smartbi 只读实测并冻结 V3.1 架构；签署 G0 & D1 国家主表、来源登记 & 复核完整率代码 & 整理 45 案例目录和缺口\\
8/17 & 决定受限来源替代 & 商务部公报导入 & 审核汇率方向测试 & 启动 20 家年报定位\\
8/18 & 评审 D3 覆盖表 & 亚洲候选宏观采集 & 建立 QA 基准 & ICT/家电企业录入\\
8/19 & 跟踪 G1 & 非洲候选宏观采集 & 检查覆盖和币制跳点 & 汽车/装备企业录入\\
8/20 & 冻结数据源版本 & 拉美候选宏观采集 & 完整率独立复算 & 基建/资源企业录入\\
8/21 & 主持 G2 样本会 & 运行 40 国选择 & 验证 15/14/11 和对照 & 企业关键数字双复核\\
8/22 & 执行 P0 三表小样、图表、三问和 XML 导出 & 政策与平行汇率 & 事件脚本初跑 & 案例证据矩阵和事件证据\\
8/23 & 评审 G3 问题 & 补采/清洗九表 & 事件合并和周期规则 & 企业画像与六家评分\\
8/24 & 批准 G3 & 冻结权威数据候选 & 资产收益、黄金分解 & 核查反例与冲突证据\\
8/25 & 建数据模型骨架 & 导出首次 Smartbi 包 & 组合情景与对称剔除 & 准备中文指标口径\\
8/26 & 完成前两页看板 & 修复导入类型/连接 & 模型敏感和反例检查 & 完成第三页与来源抽屉内容\\
8/27 & 完成第四、五页 & 更新导入清单 & 复算 100 个结果 & 配置 20 个 AI 测试卡\\
...
这是一条高风险排期。发生延期时，优先保留证据质量、40 国核心汇率/CPI、合格独立事件、最多 6 家深度企业、对称资产实验、五页故事线、来源追溯和 XML 恢复。不得通过删除反例、降低事件阈值、把年度数据复制成月度或把模拟值写成真实值赶工。
...
事件与周期 & A & C & R & C & 触发轨迹/G4\\
资产与组合 & A & C & R & C & 复算/无前视\\
Smartbi 包 & A & R & C & I & 行数哈希\\
五页看板 & R/A & C & C & C & 页面验收\\
AIChat 20 问 & A & C & C & R & 测试卡/录屏\\
...
\midrule
冻结数据包 & raw 快照、九表、维度、桥表、字典、来源 & B & 运行号、行数、哈希。\\
Python 管线 & 12 个脚本、运行器、配置、依赖锁定 & B/C & 全量重跑成功。\\
模型结果 & 风险、事件、周期、收益、情景、反例 & C & 复算与无前视报告。\\
Smartbi 资源 & 隔离项目、星座模型、指标模型、五页、AI 项目/知识库/智能体、XML & A & P0、对账、性能、隔离恢复。\\
分析报告 & 方法、结果、反例、限制、来源 & A/D & 数字追溯 100\%。\\
三分钟视频 & 问题、数据、五页故事线、结论与边界 & D & 时长和画面/声音检查。\\
...
\caption{V3.1 文档验收清单}\label{tab:document-qa}\\
\toprule
\textbf{编号} & \textbf{验收问题} & \textbf{通过证据}\\
\midrule
DOC01 & 是否仅将独立 V3.0 总计划升级为平台实测 V3.1，未覆盖 V1.1、V2.0、references 和旧 Mermaid？ & 受保护旧文件哈希前后对比。\\
DOC02 & D0—D12 是否都有负责人、输入、操作、输出、验收、失败处理和下一接收人？ & 步骤卡逐项检查。\\
DOC03 & 九张表是否均写字段、来源、作用、处理、路径和阈值？ & 字段字典和步骤映射。\\
DOC04 & 130 国、40 国、20 企业、192 月、20 事件是否未被写成已完成成果？ & 全文检索计划/现状表述。\\
DOC05 & 月度、年度和事件数据是否分表且无伪月度？ & 字段字典与频率审计。\\
DOC06 & 黄金与美元是否采用周期协同和对称剔除？ & D9、第四页与公式。\\
DOC07 & Smartbi 是否具体到 16 个数据工作簿、3 个控制工作簿、唯一前缀、星座模型、指标模型、五页、AI 对象和回退？ & 实测章、D10—D11 与操作手册。\\
DOC08 & AIChat 是否有 20 张测试卡、来源、容差和限制？ & AI-01—AI-20。\\
DOC09 & 是否包含 XML 干净恢复、性能和提交闭环？ & G5/G6 与恢复章。\\
DOC10 & PDF 是否使用 XeLaTeX 成功编译，无未定义引用、表格越界和异常空白页？ & 编译日志与逐页渲染检查。\\
DOC11 & PDF 是否为 100—140 页的可读详细计划？ & pdfinfo 页数与视觉抽检。\\
DOC12 & 最后一页是否直接显示版本号 V3.1 和 2026-08-16？ & 最后一页截图/文本提取。\\
...
\toprule
\textbf{赛题能力} & \textbf{V3.1 实施} & \textbf{验收} & \textbf{证据}\\
\midrule
数据接入与处理 & Python 采集、清洗、九表和 Excel 导入 & G1—G3 & 原始快照、脚本、QA。\\
数据建模 & 六维、九事实/逻辑表、桥表和单一星座增强模型 & 主外键、混频与膨胀检查 & 模型截图、映射表、对账。\\
可视化交互 & 五页、筛选、联动和下钻 & P01—P06 & 录屏、响应时间。\\
智能分析 & 指标模型、知识库、智能体和 AIChat 20 问 & 正确率/容差/来源/粒度边界 & 测试卡与录屏。\\
业务创新 & 企业国别风险、三层储备和周期协同 & 对称实验与反例 & 情景表、分析报告。\\
...
7 & 九表主键、来源、频率和质量通过 G3 & B/C & 待执行\\
8 & 黄金收益分解、成本、权重和无前视通过 & C/B & 待执行\\
9 & 两类资产各至少 3 个不占优窗口保留 & C/D & 待执行\\
10 & P0 小样门通过，16 个数据工作簿和 3 个控制工作簿导入前后行数、主键一致 & A/B & 待执行\\
11 & 五页筛选、联动、下钻和 10 秒性能通过 & A/D & 待执行\\
12 & AIChat 20 问、来源和容差通过 & D/A & 待执行\\
...
19 & 所有新平台对象使用 \Field{XH202612_V31_} 前缀，既存资源零改动 & A/D & 待执行\\
20 & \File{Smartbi}\textsf{模型映射}\File{_V3.1.xlsx}与平台字段、关系、页面、AI 指标逐项一致 & A/B & 待执行\\
\bottomrule
\end{longtable}
...
\begin{center}
  ...
  \begin{tabular}{p{4.2cm}p{8.0cm}}
    \toprule
    文档名称 & 亚非拉国别风险与多币种储备智能决策平台总计划书\\
    \midrule
    版本号 & \textbf{V3.1}\\
    编制日期 & 2026 年 8 月 16 日\\
    文档状态 & 数据采集、处理、验收与 Smartbi 落地平台实测定稿\\
    适用项目 & 面向中国出海企业的国别风险与应急储备智能决策平台\\
    文档边界 & V3.1 是 V3.0 经主办方实例只读核验后的平台落地更新；不覆盖 V1.1、V2.0、旧参考文献或旧 Mermaid\\
    \bottomrule
  \end{tabular}
  \vfill
  {\Large\bfseries\color{DeepNavy}版本号：V3.1\par}
  \vspace{1.0cm}
