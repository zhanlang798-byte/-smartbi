...
\hypersetup{
  ...
  citecolor=OceanBlue,
  urlcolor=OceanBlue,
  pdftitle={全球经济周期与亚非拉国别风险多币种储备智能决策平台总计划书},
  pdfsubject={全球周期、系统性危机、国别风险、储备配置与Smartbi落地},
  pdfauthor={数据创新平台项目组}
}
...
\begin{titlepage}
  ...
  \vspace{0.7cm}
  {\LARGE\color{OceanBlue}经济周期、系统性危机、数据治理与 Smartbi 落地\par}
  \vspace{1.0cm}
...
本册是一份独立、可执行的研究与实施总计划。它在 V3.1 的国别风险和多币种储备框架之外，正式增加全球经济周期量化层与历史制度压力层；不把尚未采集的数据、尚未运行的模型或尚未建设的 Smartbi 看板写成既成成果。每一步的输入、操作、输出文件、数据主键、质量阈值、失败处理和交接对象均固化为团队合同，3—4 名成员应当能够仅凭本册完成任务拆分和阶段验收。
\end{decisionbox}
...
2026 年 8 月 16 日对主办方账号完成了不写入资源的认证与能力盘点：实例为 Smartbi Insight Edition V11，已开放增强数据集、导入表/Excel、自助 ETL、指标模型、可视化看板、AIChat、项目、知识库、智能体及资源导出/迁移相关能力。结合官方 V11、数据准备与 AIChat 说明\cite{smartbiversion,smartbiprepare,smartbiaichat}，本项目的主路径据此冻结为“Python 权威 XLSX 包—导入表/Excel—星座型增强数据模型—统一指标模型—六页看板—AI 项目/知识库/智能体—XML 导出与隔离恢复”。上传、建模、看板、AIChat 数值测试和 XML 恢复尚未执行，必须先通过平台小样试跑，不能用本次只读盘点替代产品验收。
...
本册独立维护参考文献，不调用也不修改历史 \File{references.bib}。历史 V1.1、V2.0、V3.1 与旧 Mermaid 保持原状，仅作为项目演进记录。2025 年数据若在执行时尚无正式值，则保持为空并报告最新完整期，不得用预测值冒充实际值。1929—1970 年只进入历史制度与压力测试层，不伪造统一月度序列；美国、日本和欧洲只作为全球冲击源和历史对照，不进入亚非拉国别风险排名。
...
\begin{enumerate}
  ...
  \item Smartbi 星座型增强数据模型 \Field{MDL_XH202612_COUNTRY_RESERVE_V40}、指标模型 \Field{IM_XH202612_COUNTRY_RESERVE_V40}、六页看板、AI 项目/知识库/智能体与 25 个 AIChat 测试卡；
...
\toprule
\textbf{序} & \textbf{事件} & \textbf{核心分类} & \textbf{V4.0 中的作用与限制}\\
\midrule
\endfirsthead
\toprule \textbf{序} & \textbf{事件} & \textbf{核心分类} & \textbf{V4.0 中的作用与限制}\\ \midrule
\endhead
...
共享账号内任何通用名称都可能与他人资源冲突。所有项目对象统一前缀 \Field{XH202612_V40_}，显示名可以使用中文，但资源内部名和导入文件名必须保持唯一。A 在创建前登记“对象名—创建人—修改人—依赖—导出批次”，同一对象同一时段只允许一名写入者。

\begin{longtable}{p{3.4cm}p{5.4cm}p{5.2cm}}
\caption{Smartbi V4.0 权威资源命名}\label{tab:smartbi-resource-names}\\
\toprule
\textbf{对象} & \textbf{固定名称} & \textbf{作用与依赖}\\
\midrule
隔离项目/目录 & \Field{XH202612_V40} & 只承载本项目新资源；不接管账号中任何既存对象。\\
增强数据模型 & \Field{MDL_XH202612_COUNTRY_RESERVE_V40} & 星座模型；共享国家、月、年、企业、资产、事件六类维度。\\
指标模型 & \Field{IM_XH202612_COUNTRY_RESERVE_V40} & 固化指标代码、中文名、公式、默认聚合、计价口径、时间粒度和来源。\\
六页看板 & \Field{DB01_XH202612_RISK_OVERVIEW_V40} 至 \Field{DB06_XH202612_GLOBAL_CYCLE_CRISIS_V40} & 依次对应风险全景、国别事件、企业暴露、周期实验室、决策证据、全球周期与历史危机。\\
AI 项目 & \Field{PRJ_XH202612_COUNTRY_RESERVE_V40} & 绑定指标模型、知识库和测试问题，不直接面向未治理的原始事实表。\\
知识库 & \Field{KB_XH202612_COUNTRY_RESERVE_V40} & 收纳术语、数据字典、危机规则、来源等级、约束路由、45 案例和历史制度说明。\\
智能体 & \Field{AGT_XH202612_COUNTRY_RESERVE_V40} & 只回答经授权粒度的问题；输出指标口径、截止期、来源和限制。\\
恢复包 & \Field{PKG_XH202612_COUNTRY_RESERVE_V40} & XML、依赖清单、数据包、映射表、版本说明、哈希和恢复记录的统一批次。\\
\bottomrule
\end{longtable}

\section{输出格式、模型关系与调用合同}

Python 输出分为 18 个可导入数据工作簿和 3 个控制工作簿。18 个数据工作簿由 11 张权威逻辑表、六张维度表和一张企业—地区桥表构成；\Field{dim_year.xlsx} 专门连接年度投资、政策和企业披露，禁止把年度值复制成 12 个月。三个控制工作簿是 \textsf{数据字典}\File{_V4.0.xlsx}、\File{Smartbi}\textsf{导入行数与哈希清单}\File{.xlsx} 和 \File{Smartbi}\textsf{模型映射}\File{_V4.0.xlsx}。

\File{Smartbi}\textsf{模型映射}\File{_V4.0.xlsx} 至少包含 \Field{import_order}、\Field{file_name}、\Field{sheet_name}、\Field{smartbi_resource_name}、\Field{grain}、\Field{primary_key}、\Field{field_name}、\Field{dimension_measure_role}、\Field{data_type}、\Field{relationship}、\Field{dashboard_page}、\Field{ai_metric_code}、\Field{refresh_mode}、\Field{source_id} 和 \Field{run_id}。团队不得只凭口头约定在平台中猜字段角色。
...
在完整 D10 导入前，A 必须取得项目组对写入范围的确认，然后只在 \Field{XH202612_V40} 隔离目录创建一个可删除的小样批次。小样包含 \Field{dim_country}、\Field{dim_date}、\Field{country_monthly_risk} 和 \Field{global_cycle_month} 四张脱敏示例表，覆盖至少 3 国、24 个月及全球周期的汇率/CPI/来源/质量/状态字段。依次验证 Excel 类型识别、维度连接、一个 KPI、两张折线图、一个国家筛选器、一个周期筛选器、五条 AI 问题、XML 导出和可用目标空间中的恢复。
...
\CardRow{输入}{赛题原文、现有 45 份案例与其中 17 份宏观案例、历史计划书、商务部公报结构、国别与全球周期候选指标、Smartbi 试用环境约束。}
\CardRow{操作}{建立 \File{config/project_scope_v40.yaml}、\File{config/indicator_catalog_v40.xlsx}、\File{config/company_master_v40.xlsx}、\File{config/global_cycle_rules_v40.yaml}、\File{config/historical_event_master_v40.xlsx} 和 \File{config/source_priority_v40.xlsx}。逐项冻结研究对象、1971/2010/1929 三类日期边界、币种方向、原始频率、主键、国别事件阈值、全球周期多标签与优先级、三层储备、三种计价、历史可比性、许可规则、禁止使用未来信息规则和 QA 阈值。所有变更通过变更单记录提出人、理由、影响表、批准人和生效运行号。}
\CardRow{输出路径}{\File{config/} 下四份配置；\File{data/qa/D0_scope_signoff_20260815.xlsx}；范围冻结纪要。}
\CardRow{验收}{130、40、20、192 个月、1971—2025、1929—2025、17 个历史锚点、11 张权威表、六页和 25 问等口径在全部配置中一致；汇率方向明确为本币/美元；年度/季度指标不得冒充月度观测；每个字段有类型、频率、单位、许可和责任人；四名角色或实际三名成员完成签字。}
...
\CardRow{输入}{\File{config/source_priority_v40.xlsx}、各机构数据页面/API、许可证或使用说明、截至日期 2025-12-31。}
...
\CardRow{输出路径}{\File{config/selected_40_countries_v40.csv}、\File{data/qa/country_selection_scorecard.xlsx}、\File{data/qa/excluded_country_reasons.xlsx}。}
...
\CardRow{输出路径}{\File{templates/policy_event_manual_entry_v40.xlsx}、\File{data/staging/country_policy_coded.parquet}、\File{data/curated/country_policy_year.xlsx}。}
...
\CardRow{输出路径}{\File{templates/company_disclosure_v40.xlsx}、\File{data/staging/company_disclosure_checked.parquet}、\File{data/curated/company_overseas_exposure.xlsx}。}
...
\CardRow{输出路径}{\File{templates/case_evidence_v40.xlsx}、\File{templates/event_evidence_v40.xlsx}、\File{data/staging/asset_prices_standardized.parquet}、\File{data/curated/case_evidence.xlsx}。}
...
\CardRow{输入}{宏观危机篇 17 份档案、其原始来源、NBER/央行/监管/学术资料、制度与黄金政策资料，以及《广场协议》和日本长期停滞的权威回顾。}
\CardRow{操作}{运行 \File{scripts/09a_build_historical_crisis_events.py} 导入双人复核模板。每项区分 crisis、policy node、regime change 和 counterexample；记录起止日、事件簇、主/次状态、货币制度、资本管制、黄金合法持有、市场可交易性、可用资产、政策响应、传导渠道、证据等级、页码、可比性等级与现代回测许可。1985 年《广场协议》只作为日本事件簇的政策节点，不单独计为危机。}
\CardRow{输出路径}{\File{templates/historical_crisis_event_v40.xlsx}；\File{data/curated/historical_crisis_event.xlsx}；\File{data/qa/historical_event_evidence_audit.xlsx}；\File{data/qa/historical_cluster_dedup.xlsx}。}
\CardRow{验收}{17 份宏观档案全部有唯一事件或节点记录并完成来源复核；《广场协议》、日本泡沫和长期停滞同簇且不重复计数；1929—1970 年的 \Field{modern_backtest_allowed=0}；每个历史资产比较都有制度和可交易性限定。}
...
\CardRow{验收}{11 表主键重复率 0、主键空值率 0；正式数值 source ID 覆盖率 100\%；40 国汇率和 CPI 各满足 154/192；全球周期核心适用指标完整率不低于 90\%；频率审计无年度/季度伪月度；1929—1970 未生成统一月表；货币改制机械跳点不被当危机；受限许可原始序列不进入提交包；代理、模拟、插值和真实记录可分别筛选。}
...
\CardRow{输出路径}{18 个 \File{data/smartbi/*.xlsx} 数据文件，以及 \File{data/smartbi/}\textsf{数据字典}\File{_V4.0.xlsx}、\File{data/smartbi/Smartbi}\textsf{导入行数与哈希清单}\File{.xlsx}、\File{data/smartbi/Smartbi}\textsf{模型映射}\File{_V4.0.xlsx}。}
...
\CardRow{操作}{在 \Field{XH202612_V40} 隔离目录中，按映射表通过“导入表/Excel”建立数据对象；规则式导入不可用时才回退到自助 ETL 的 Excel/导入文件节点，业务主题只作为兼容回退。建立星座型增强数据模型 \Field{MDL_XH202612_COUNTRY_RESERVE_V40}，由六张维度共享连接 11 张事实/逻辑表；事实表之间禁止直接多对多。随后建立 \Field{IM_XH202612_COUNTRY_RESERVE_V40}，固化指标代码、中文别名、默认聚合、计价口径和时间粒度；再创建六页看板、AI 项目、知识库和智能体。}
...
\CardRow{操作}{导出项目目录、星座型增强数据模型、指标模型、六页看板、AI 项目、知识库、智能体及其数据对象的 XML/迁移资源与依赖清单；在未加载本项目资源的隔离目标恢复，记录环境版本、导入步骤、耗时、警告和截图；执行 12 个关键看板路径与 25 个 AI 问题的恢复后抽验。完成分析报告、三分钟以内视频、操作指南、示例数据、字段字典、来源说明、合规声明和最终哈希清单。运行 \File{scripts/12_generate_qa_report.py} 汇总全链路验收。}
...
\CardRow{验收}{XML 干净恢复成功；六页和关键指标可用；视频不超过 3 分钟；报告中的关键数字全部可追溯；提交包无临时锁文件、登录凭据和绝对本机路径；最终清单记录每个文件大小与 SHA-256。}
...
\begin{verbatim}
python scripts/run_pipeline.py --as-of 2025-12-31 --run-id 20260831_v40
\end{verbatim}
...
页面固定资源名为 \Field{DB06_XH202612_GLOBAL_CYCLE_CRISIS_V40}。它不把美国、日本或欧洲纳入亚非拉国别排名，而是把全球冲击源、历史机制、亚非拉传导和储备工具表现放到同一解释路径：先识别全球状态，再观察哪些国家通过汇率、贸易、融资、资本流动或银行支付通道受到影响，最后进入第四页对比美元与黄金的边际贡献。
...
\begin{enumerate}
  \item 先查重并创建隔离目录 \Field{XH202612_V40}，登记资源锁；不得复用或修改登录前已有对象；
  ...
  \item 每个对象采用表\ref{tab:smartbi-resource-names}和 \File{Smartbi}\textsf{模型映射}\File{_V4.0.xlsx}的唯一名称、角色、连接与刷新方式；
...
干净环境是未加载 \Field{XH202612_V40} 项目数据、模型、指标、看板、知识库、智能体和 AIChat 配置的新租户或经主办方确认的隔离目标。可以保留产品本身和必要账号，但不得预先存在同名前缀对象。同一非空工作区内覆盖旧对象不算干净恢复。恢复人最好不是原搭建人，以验证操作指南是否足够；若主办方不给第二目标空间，必须把恢复标为未验收并说明外部依赖。
...
\begin{enumerate}
  ...
  \item 按操作指南连接或导入 V4.0 的 18 个数据工作簿与 3 个控制工作簿，不使用源环境本机绝对路径；
...
\endhead
8/15 & 保留 V3.1 历史基线和旧文件哈希 & 核对 130 国和国别指标源 & 固定国别事件/组合公式 & 固定企业与案例模板\\
8/16 & 冻结 V4.0 双层危机架构、六页与 25 问；签署 G0 & 增加全球周期来源和许可清单 & 固定多标签、主状态与历史类比规则 & 将 17 宏观案例映射到历史事件模板\\
8/17 & 决定受限信用序列替代和分发边界 & 商务部公报导入；全球数据试采 & 审核汇率、NFCI 分位、回撤测试 & 启动 20 家年报与历史来源定位\\
...
\caption{V4.0 文档验收清单}\label{tab:document-qa}\\
\toprule
\textbf{编号} & \textbf{验收问题} & \textbf{通过证据}\\
\midrule
DOC01 & 是否新建独立 V4.0，且 V1.1、V2.0、V3.1、旧 references 和旧 Mermaid 均保持原哈希？ & 受保护旧文件哈希前后对比。\\
...
DOC12 & 最后一页是否直接显示版本号 V4.0 和 2026-08-16？ & 最后一页截图/文本提取。\\
...
\toprule
\textbf{赛题能力} & \textbf{V4.0 实施} & \textbf{验收} & \textbf{证据}\\
\midrule
...
19 & 所有新平台对象使用 \Field{XH202612_V40_} 前缀，既存资源零改动 & A/D & 待执行\\
20 & \File{Smartbi}\textsf{模型映射}\File{_V4.0.xlsx}与平台字段、关系、页面、AI 指标逐项一致 & A/B & 待执行\\
...
25 & V1.1、V2.0、V3.1 与旧 Mermaid 哈希保持不变 & A & 待执行\\
\bottomrule
\end{longtable}
...
\begin{center}
  ...
  \begin{tabular}{p{4.2cm}p{8.0cm}}
    ...
    文档状态 & 全球经济周期与系统性危机融合研究与实施计划稿\\
    适用项目 & 面向中国出海企业的国别风险与应急储备智能决策平台\\
    文档边界 & V4.0 为独立总计划书；不覆盖 V1.1、V2.0、V3.1、旧参考文献或旧 Mermaid\\
    \bottomrule
  \end{tabular}
  \vfill
  {\Large\bfseries\color{DeepNavy}版本号：V4.0\par}
  \vspace{1.0cm}
