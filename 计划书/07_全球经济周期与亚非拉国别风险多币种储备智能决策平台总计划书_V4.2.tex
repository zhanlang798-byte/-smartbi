...
\hypersetup{
  ...
  citecolor=OceanBlue,
  urlcolor=OceanBlue,
  pdftitle={全球经济周期与亚非拉国别风险多币种储备智能决策平台总计划书 V4.2},
  pdfsubject={网页数据可用化、赛题逐条落地、全球周期、国别风险与Smartbi执行交接},
  pdfauthor={数据创新平台项目组}
}
...
\begin{titlepage}
  ...
  \vspace{0.7cm}
  {\LARGE\color{OceanBlue}网页数据可用化、赛题逐条落地与 Smartbi 执行\par}
  \vspace{0.35cm}
  {\Large\bfseries\color{GoldLine}V4.2 数据可用化与评分执行版\par}
  \vspace{1.0cm}
...
本册是在 V4.1 交接基线上新建的 V4.2 数据可用化与评分执行版。它保留 130 国、40 国、20 家企业、全球周期、历史危机、多币种储备和 Smartbi 六页产品的主体设计，同时把旧归档中“文件已下载”与“内容可使用”分开：688 项来源均取得获取、内容、提取和正式用途四态；159 个本地 HTML 逐项审计；开放底层资源按许可补取并转成可追溯表格或正文。赛题五类评分原文保持不动，另以操作卡绑定所需字段、脚本、交付物、Smartbi 页面和验收证据。每一步的输入、命令、输出文件、质量阈值、失败处理和下一接收人均固化为团队合同。
\end{decisionbox}
...
\mainmatter
\input{v42_operational_chapters.tex}
...
\bibitem{archivev11}
数据创新平台项目组：《调研资料归档 V1.1：网页数据可用化增量包》，运行号 \Field{20260816_web_usability_v11}，包括688项四态动作清单、159项本地HTML处理矩阵、开放底层资源、正文与表格提取、结构化资源清单和失败任务卡，2026-08-16。

\bibitem{secapi}
U.S. Securities and Exchange Commission, EDGAR Application Programming Interfaces, \url{https://www.sec.gov/search-filings/edgar-application-programming-interfaces}, accessed 2026-08-16.

\bibitem{treasuryxml}
U.S. Department of the Treasury, Treasury Daily Interest Rate XML Feed, \url{https://home.treasury.gov/treasury-daily-interest-rate-xml-feed}, accessed 2026-08-16.

\bibitem{bisapi}
Bank for International Settlements, BIS Statistics API Documentation, \url{https://stats.bis.org/api-doc/v2/}, accessed 2026-08-16.
...
\begin{center}
  ...
  \begin{tabular}{p{4.2cm}p{8.0cm}}
    ...
    文档状态 & 网页数据可用化、评分逐条落实与执行交接稿\\
    适用项目 & 面向中国出海企业的国别风险与应急储备智能决策平台\\
    文档边界 & V4.2 为独立数据可用化与评分执行版；不覆盖 V1.1—V4.1、调研归档 V1.0、旧参考文献或旧 Mermaid\\
    \bottomrule
  \end{tabular}
  \vfill
  {\Large\bfseries\color{DeepNavy}版本号：V4.2\par}
  \vspace{1.0cm}
